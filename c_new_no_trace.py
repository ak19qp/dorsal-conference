...
f = open("cfdata.csv", "w")
f.write("Metric1,Metric2,Metric3,|,Ceiling1,Ceiling2,Ceiling3,|,Floor1,Floor2,Floor3,|,CTriggered1,CTriggered2,CTriggered3,|,FTriggered1,FTriggered2,Ftriggered3,|,ARIMAActionTaken1,ARIMAActionTaken2,ARIMAActionTaken3,|,CORRActionTaken1,CORRActionTaken2,CORRActionTaken3\n")
f.close()
...
while i < 100:
	...
	#saving correlations
	if i >= sample_size:
		datasetselected = df[['cpu_percent', 'memory_percent', 'disk_usage_percent']]
		correlation_mat = datasetselected.corr()
	...
	f = open("cfdata.csv", "a")
	f.write(str(dataset_cpu1[0]) + "," + str(dataset_ram1[0]) + "," + str(dataset_hdd1[0]) + ",|,")
	...
	while counter < no_of_metrics:
		...
		if ARIMAactionTaken:
			...
			arima_action_taken_string = arima_action_taken_string + "0,"
	...
	f.write(ceiling_hits_string + ",|," + floor_hits_string + ",|," + arima_action_taken_string + ",|," + corr_action_string)
	f.write("\n")
	f.close()
